...
namespace RCF {
    ...
    void printToOstream(RCF::MemOstream & os, std::uint16_t n)
    {
        char buffer[50] = {0};
        sprintf(buffer, "%u", std::uint32_t(n));
        os << buffer;
    }

    void printToOstream(RCF::MemOstream & os, std::uint32_t n)
    {
        char buffer[50] = {0};
        sprintf(buffer, "%u", n);
        os << buffer;
    ...
    void printToOstream(RCF::MemOstream & os, std::uint64_t n)
    {
        char buffer[50] = {0};
...
#ifdef __cpp_char8_t
    void printToOstream(MemOstream& os, const std::u8string& t)
    {
    ...
    LogManager & LogManager::instance()
    {
        if (!gpLogManager)
        {
            throw Exception(RcfError_RcfNotInitialized);
        }
        return *gpLogManager;
    }
    ...
    std::uint32_t getCurrentMsValue()
    {
        SYSTEMTIME st; 
        GetSystemTime(&st);
