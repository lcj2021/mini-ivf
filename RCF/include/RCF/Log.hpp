...
#include <RCF/VariableArgMacro.hpp>
...
namespace RCF { 
    ...
    class LogTarget;
    typedef std::shared_ptr<LogTarget> LogTargetPtr;

    /// Base class for log targets. 
    
    /// Log targets are passed to the enableLogging() function, to configure RCF logging.
    ...
    typedef std::function<void(const ByteBuffer &)> LogFunctor;
    ...
    typedef std::function<void(const LogEntry &, ByteBuffer&)> LogFormatFunctor;

    class RCF_EXPORT Logger : public std::enable_shared_from_this<Logger>
    {
    public:
    ...
    #define UTIL_LOG(name, level)                                               \
        if (RCF::LogManager::instance().isEnabled(name, level))                 \
            UTIL_LOG_GCC_33_HACK RCF::VariableArgMacro<RCF::LogVarsFunctor>(    \
                name, level, __FILE__, __LINE__, RCF_CURRENT_FUNCTION)          \
