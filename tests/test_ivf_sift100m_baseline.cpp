...
int main(int argc, char* argv[]) {
    ...
    std::tie(nb, D) = load_from_file_binary<uint8_t>(database, db_path + "/base.bvecs");

    std::vector<float> query;
    load_from_file_binary<uint8_t>(query, query_path + "/query.bvecs");

    std::vector<int> gt;
    auto [n_gt, d_gt] = load_from_file_binary<int>(gt, query_path + "/gt.ivecs");

    std::vector<float> gt_dist;
    load_from_file_binary<float>(gt_dist, query_path + "/dist.fvecs");
