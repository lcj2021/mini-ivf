...
const std::string name              = "IVFPQ4SIFT1M";
size_t nt                           = 1'000'000;   
size_t kc                           = 4096;
size_t kp                           = 256;
size_t mp                           = 64;
...
const std::string index_path        = "../../data/index/sift1m";
const std::string db_path           = "../../data/db/sift1m";
...
int main(int argc, char *argv[]) 
{
    ...
    if (argc == 3 && std::string(argv[2]) == "check") {
        ...
        ivfpq->TopWID(nprobe, queries_nest, books);
        ivfpq->TopKID(k, queries_nest, books, nnid, dists);
        timer.Stop();
        ...
        {
            std::unordered_set<index::vector_id_t> S(gt.begin() + q * d_gt, gt.begin() + (q + 1) * d_gt);
